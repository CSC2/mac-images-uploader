...
// in simulator we redefine to format for mac_images_uploader Developer console
...
{\
	NSString * exceptionName = [@"org.mac_images_uploader." stringByAppendingString:NSStringFromClass([self class])];\
	TiExceptionThrowWithNameAndReason(exceptionName,reason,subreason,location);\
}\
\
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.mac_images_uploader";\
	TiExceptionThrowWithNameAndReason(exceptionName,reason,subreason,location);\
}\
...
enum {
	UImac_images_uploaderNativeItemNone = -1, 
	UImac_images_uploaderNativeItemSpinner = -2,
	UImac_images_uploaderNativeItemProgressBar = -3,
	
	UImac_images_uploaderNativeItemSlider = -4,
	UImac_images_uploaderNativeItemSwitch = -5,
	UImac_images_uploaderNativeItemMultiButton = -6,
	UImac_images_uploaderNativeItemSegmented = -7,
	
	UImac_images_uploaderNativeItemTextView = -8,
	UImac_images_uploaderNativeItemTextField = -9,
	UImac_images_uploaderNativeItemSearchBar = -10,
	
	UImac_images_uploaderNativeItemPicker = -11,
	UImac_images_uploaderNativeItemDatePicker = -12,
	
	UImac_images_uploaderNativeItemInfoLight = -13,
	UImac_images_uploaderNativeItemInfoDark = -14,
	
	UImac_images_uploaderNativeItemDisclosure = -15,
	
	UImac_images_uploaderNativeItemContactAdd = -16
};
...
/**
 *	TiThreadPerformOnMainThread should replace all mac_images_uploader instances of
