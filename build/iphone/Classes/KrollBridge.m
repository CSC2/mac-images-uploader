...
NSString * mac_images_uploader$ModuleRequireFormat = @"(function(exports){"
...
@implementation mac_images_uploaderObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_mac_images_uploader);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create mac_images_uploader global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "mac_images_uploader" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_mac_images_uploader = [[mac_images_uploaderObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_mac_images_uploader];
	
	NSString *_mac_images_uploaderNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _mac_images_uploaderNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_mac_images_uploader valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:mac_images_uploader$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.mac_images_uploader.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.mac_images_uploader.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
